/**
 * [English]
 * This file contains the "Marked" library (https://github.com/markedjs/marked).
 * It is a popular tool used to convert Markdown text (like **bold** or # Header) into HTML (like <b>bold</b> or <h1>Header</h1>).
 * 
 * Why does this code look so messy?
 * This file is "minified" (.min.js). All spaces, newlines, and comments have been removed, and variable names shortened.
 * This is done to reduce the file size so it loads faster in your browser.
 * It is NOT meant to be read or edited by humans.
 * 
 * [中文说明]
 * 这个文件包含了 "Marked" 库 (https://github.com/markedjs/marked)。
 * 它是一个流行的工具，用于将 Markdown 文本（例如 **加粗** 或 # 标题）转换为 HTML（例如 <b>加粗</b> 或 <h1>标题</h1>）。
 * 
 * 为什么代码看起来这么乱？
 * 这个文件是经过 "压缩混淆" 的 (.min.js)。所有的空格、换行符和注释都被移除了，变量名也被缩短了。
 * 这样做是为了减小文件体积，让它在浏览器中加载得更快。
 * 它不适合人类阅读或编辑。
 */
/**
